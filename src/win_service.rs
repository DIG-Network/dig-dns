...
/// The actual service body: register the control handler, report `Running`, serve both `.dig`
/// resolution paths until `Stop`, then report `Stopped`.
fn run_service() -> std::io::Result<()> {
    let config = config::from_env(|k| std::env::var(k).ok())
        .map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidInput, e.to_string()))?;

    // The control handler signals `Stop` over this channel; the server's shutdown future
    // waits on it.
    ...
    // Tell the SCM we are running (so it does not time out with 1053); accept STOP.
    status_handle
    ...
    // Report stopped regardless of the serve result; carry a non-zero exit on error so the SCM
    // (and `sc query`) reflect a failed run.

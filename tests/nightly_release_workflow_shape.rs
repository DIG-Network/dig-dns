...
/// A workflow file under `.github/workflows/`, resolved relative to this crate. `dig-dns` is a
/// single-package repo, so `CARGO_MANIFEST_DIR` IS the repo root.
...
fn stable_job_is_reachable_only_from_manual_dispatch_never_from_schedule() {
    ...
    // never accepts the `schedule` trigger as an alternative (CLAUDE.md §3.6-A; dig_ecosystem#698,
    // dig-dns#27). A stable `vX.Y.Z` is permanent and reaches every installed host through the
    // signed update feed — cutting one must be a decision a person makes, never something the
    // midnight cron does unattended.
    //

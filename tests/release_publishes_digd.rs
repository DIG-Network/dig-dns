//! Regression guard for the release workflow (`.github/workflows/release.yml`).
//!
...
//!
//! This fast, cross-OS test asserts the committed workflow still builds and stages `digd`, so a
//! well-meaning edit that drops the alias from the release is caught at PR time — long before a
//! tag release would silently ship without the `digd` asset. It mirrors the repo's existing
//! committed-artifact guard (`wix_manifest_wellformed.rs`).

use std::path::PathBuf;

/// The release workflow source, resolved from the crate root.
